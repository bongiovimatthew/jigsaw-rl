...
class A3CFF(chainer.ChainList, a3c.A3CModel):
    ...
    def pi_and_v(self, state):
        print(state.shape)
        print(self.head.n_input_channels )
        print(self.head.n_output_channels )
        out = self.head(state)
...
def main():
    ...
    n_actions = PuzzleEnvironment().action_space.n
    ...
    fake_obs = chainer.Variable(
        np.zeros((1, 84, 84), dtype=np.float32)[None],
        name='observation')
