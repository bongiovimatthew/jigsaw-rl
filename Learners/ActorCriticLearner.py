...
class ActorCriticLearner:
    ...
    def execute_agent(env, batch_length, game_length, total_max_moves, gamma, lr):
        agent = ActorCriticLearner.create_agent(env, batch_length, game_length, total_max_moves, gamma, lr)
        agent.run()
    ...
    def process_img(observation):
        img_final = np.array(Image.fromarray(observation, 'RGB').convert("L").resize((ActorCriticLearner.STATE_WIDTH, ActorCriticLearner.STATE_HEIGHT), Image.ANTIALIAS))
        img_final = np.reshape(img_final, (1, ActorCriticLearner.STATE_WIDTH, ActorCriticLearner.STATE_HEIGHT))
...
class ActorCriticAgent: 
    ...
    def play_game_for_a_while(self):
        ...
        if self.is_terminal:
            if self.debug_mode: 
                logger.log_state_image(self.current_state, self.total_step_count, -1, (ActorCriticLearner.STATE_WIDTH, ActorCriticLearner.STATE_HEIGHT))
            self.current_state = ActorCriticLearner.env_reset(self.env, self.queue)
            self.episode_step_count = 0
            self.is_terminal = False
                    
        self.epsilon = max(0.1, 1.0 - (((1.0 - 0.1)*1.5) / self.total_max_moves) * self.total_step_count) # first decreasing, then it is constant
        batch_count = 0 
        ...
        while not (self.is_terminal or batch_count - batch_count_start == self.batch_length):
            ...
            if self.debug_mode: 
                ...
                logger.log_state_image(self.current_state, self.episode_step_count, self.learner_id, action, (ActorCriticLearner.STATE_WIDTH, ActorCriticLearner.STATE_HEIGHT))
